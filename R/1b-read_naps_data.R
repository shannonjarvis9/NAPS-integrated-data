...
getHeader <- function(file) {
  ...
  while (found_header == FALSE & i < nrow(file)) {
    i <- i + 1
    if (any(grep(
      paste0(c("NAPS", "Date") , sep = "", collapse = "|"),
      file[i, ],
      ignore.case = TRUE,
      value = FALSE
    ))) {
...
readData_xlsx_sheets <- function(dir){
  ...
  # Iterate through the files, adding to the list by station number
...
readData_xls_nosheets <- function(dir){
  ...
  # iterate through the files, adding them to a list 
...
getSamplerInfo <- function(dir){
  ...
  # Iterate through the files, looking for the row that has the sampler info
  for(i in 1:length(files)){
